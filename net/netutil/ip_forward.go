...
func CheckIPForwarding(routes []netip.Prefix, state *netmon.State) (warn, err error) {
	...
	if runtime.GOOS != "linux" {
		...
		case "dragonfly", "freebsd", "netbsd", "openbsd":
			warnings = append(warnings, fmt.Sprintf("Subnet routing and exit nodes only work with additional manual configuration on %v, and is not currently officially supported.", runtime.GOOS))
		}
		return nil, nil
	}
	if state == nil {
	...
	v4e, err := ipForwardingEnabled(ipv4, "")
	if err != nil {
	...
	v6e, err := ipForwardingEnabled(ipv6, "")
	if err != nil {
		return nil, fmt.Errorf("Couldn't check system's IP forwarding configuration, subnet routing/exit nodes may not work: %w%s", err, kbLink)
	}
	...
	for _, iface := range state.Interface {
		...
		v4e, err := ipForwardingEnabled(ipv4, iface.Name)
		if err != nil {
			return nil, fmt.Errorf("Couldn't check system's IP forwarding configuration, subnet routing/exit nodes may not work: %w%s", err, kbLink)
		} else if !v4e {
...
func CheckReversePathFiltering(state *netmon.State) (warn []string, err error) {
	...
	allSetting, err := reversePathFilterValue("all")
	if err != nil {
	...
	for _, iface := range state.Interface {
		...
		iSetting, err := reversePathFilterValue(iface.Name)
		if err != nil {
